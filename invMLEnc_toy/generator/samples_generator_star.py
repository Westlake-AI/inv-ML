...
def make_swiss_roll(n_samples=100, noise=0.0, random_state=None, remove=False, scale=1.0, center=None, r=None):
    ...
    # length, _ = integrate.quad(f, 1.5 * np.pi, t)
    ...
    if remove == 'star':
        ...
        def isInside(p1, p2, p3, p):
            ...
            if abs(A - (A1 + A2 + A3)) < 0.1:
                ...
                # print(A - A1 + A2 + A3)
                # print(x1, y1, x2, y2, x3, y3, x, y)
                # print(A, A1, A2, A3, 'notin')
                return False

        def isInstar(center, r, XX):

            list_zuobiao_out = [
                (r*math.cos((90+72*x)/180*3.14), r*math.sin((90+72*x)/180*3.14)) for x in [0,1,2,3,4]
            ]
            r = (r*math.sin((18)/180*3.14)) / math.sin((126)/180*3.14)
            list_zuobiao_in = [
                (r*math.cos((126+72*x)/180*3.14), r*math.sin((126+72*x)/180*3.14)) for x in [0,1,2,3,4]
            ]
            list_zuobiao_out = np.array(list_zuobiao_out) + np.array(center)
            list_zuobiao_in = np.array(list_zuobiao_in)+ np.array(center)
            # plt.plot(list_zuobiao_in[:, 0], list_zuobiao_in[:, 1], 'or')
            # plt.plot(list_zuobiao_out[:, 0], list_zuobiao_out[:, 1], 'og')
            
            # for i in range(5):
            #     plt.text(list_zuobiao_out[i][0], list_zuobiao_out[i][1], str(i))
            #     plt.text(list_zuobiao_in[i][0], list_zuobiao_in[i][1], str(i))
...
def make_s_curve(n_samples=100, noise=0.0, random_state=None, remove=False, scale=1.0):
    ...
    if remove:
        ...
        star = []
        for i in range(475):
            star.append(1.55/474*i)
        for i in range(295):
            star.append(6.55 - 4.05/294*i)
        for i in range(475):
            star.append(2.5 + 1.545/474*i)
        star = np.array(star) * scale
        ...
        for i in range(295):
            stars.append(0)
        for i in range(475):
            stars.append(4.05/474*i)
        stars = np.array(stars) * scale

        y_index = np.argsort(t)
        for i in range(len(X)):
            if X[i, 1] > 1.1 and X[i, 1] < 1.2 and y_index[i] > n_samples // 10 * 4 and y_index[i] < n_samples // 10 * 5:
                kkk = i
                # print(kkk)
                break

        for i in range(len(X)):
            if t[i] - t[kkk] < 2 and t[i] - t[kkk] > 0:
                index = int((t[i] - t[kkk]) // (0.01/12.45*2))
                if index < 770:
                    if abs(X[i, 1] - X[kkk, 1]) < star[index]:
                        pass
                    else:
                        XX.append(X[i, :])
                        tt.append(t[i])
                else:
                    if abs(X[i, 1] - X[kkk, 1]) < star[index] and abs(X[i, 1] - X[kkk, 1]) > stars[index]:
                        pass
                    else:
                        XX.append(X[i, :])
                        tt.append(t[i])
